	...
	switch ($blockname) {
		...
		case 'profile':
			...
			if ($space['self']) {
				...
				if($showmagicgift) {
					$html .= '<li class="ul_magicgift"><div style="'.'background: url('.STATICURL.'image/magic/gift.small.gif) no-repeat 100% 50%;'.'">';
					if($magicinfo) {
			...
			if(!empty($managehtml)) {
				...
				$html .= '</ul>';
				if($_G['adminid'] == 1) {
					$html .= '<ul id="umanageli_menu" class="p_pop" style="width: 120px; display:none;">';
