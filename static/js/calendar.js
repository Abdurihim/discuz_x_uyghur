...
function loadcalendar() {
	...
	s += '<tr align="center" id="calendar_week"><td onclick="refreshcalendar(yy, mm-1)" title="ئالدىنقى ئاي" style="cursor: pointer;"><a href="javascript:;">&laquo;</a></td><td colspan="5" style="text-align: center"><a href="javascript:;" onclick="showdiv(\'year\');doane(event)" class="dropmenu" title="يىلنامىنى چېكىڭ" id="year"></a>&nbsp; - &nbsp;<a id="month" class="dropmenu" title="ئاي نامىنى چېكىڭ" href="javascript:;" onclick="showdiv(\'month\');doane(event)"></a></td><td onclick="refreshcalendar(yy, mm+1)" title="كېيىنكى ئاي" style="cursor: pointer;"><a href="javascript:;">&raquo;</a></td></tr>';
	s += '<tr id="calendar_header"><td>يە</td><td>دۈ</td><td>سە</td><td>چا</td><td>پە</td><td>جۈ</td><td>شە</td></tr>';
	for(var i = 0; i < 6; i++) {
		s += '<tr>';
		for(var j = 1; j <= 7; j++)
	...
	s += '<span id="halfhourselector"><select id="minutehalfhourly" onchange=\'this.value=this.value > 59 ? 59 : zerofill(this.value);controlid.value=controlid.value.replace(/(\\d+\:)\\d+/ig, "$1"+this.value)\'><option value="00">00</option><option value="30">30</option></select> </span>';
	s += '</td><td align="right" colspan="3"><button class="pn" onclick="confirmcalendar();"><em>جەزىملەش</em></button></td></tr>';
	s += '</table></div></div>';
	...
	for(var k = 1; k <= 12; k++) {
		s += '<a href="javascript:;" onclick="refreshcalendar(yy, ' + (k - 1) + ');$(\'calendar_month\').style.display=\'none\'"><span' + (today.getMonth()+1 == k ? ' class="calendar_today"' : '') + ' id="calendar_month_' + k + '">' + k + ( k < 10 ? '&nbsp;' : '') + ' ئاي</span></a><br />';
	}
	s += '</div>';
