	...
	x.getJSON(url+'&_makehtml&r='+(+ new Date()), function(ret){
		...
		if(ret && (ret=ret['data']) && ret['status'] == 'html_ok') {
			if(obj) {
				...
				obj.innerHTML = '<a href="'+ret['path']+'" target="_blank">'+title+'ھاسىللاش ئوڭۇشلۇق بولدى </a>';
			}
			if(ret['nexturl']) {
				if(obj) {
					obj.style.color = 'green';
					obj.innerHTML = 'ھاسىللاش'+title+(Math.round((ret['current']/ret['count'])*100))+'%';
				}
			...
			if(obj) {
				obj.style.color = 'red';
				obj.innerHTML = title+'ھاسىللاش مەغلۇب بولدى ';
			}
		}
	});
...
function make_html_batch(url, ids, callback, dom, single) {
	...
	if(this.single) {
		...
		progress_bar(cent);
		child.innerHTML = this.dom.getAttribute('mktitle')+'ھاسىللاشنى باشلاش';
		this.dom.innerHTML = '';
		this.dom.appendChild(child);
		this.make(id, child);
		this.child = child;
		var child2 = document.createElement('div');
		child2.innerHTML = '<a href="javascript:void(0);" id="mk_goon">ئەگەر تور كۆرگۈچىڭىز يېڭىلانمىسا ، بۇ يەرنى چېكىڭ ....</a>';
		...
		x.getJSON(this.url+id+'&_makehtml&r='+(+ new Date()), function(ret){
			if(ret && (data=ret['data']) && data['status'] == 'html_ok') {
				...
				if(!makehtml_error) {
					...
					makehtml_error.innerHTML = 'خاتالىق ئۇچۇرى';
			...
			if(obj.single) {
				...
				child.innerHTML = '<div class="mk_msg">'+'<a href="'+data['path']+'" target="_blank">'+obj.dom.getAttribute('mktitle')+'</a>ھاسىللاش تامام'+'</div>';
				if(obj.callback) {
				...
				progress_bar(cent);
				var str = 'نۆۋەتتىكى ھاسىللاشقا بولىدىغان'+obj.count+'دانە'+obj.dom.getAttribute('mktitle')+'ھۆججەت بار  ، بۇنىڭدىن ھاسىللانغىنى '+obj.makedcount+'دانە ،';
				if(cent != 100) {
					child.innerHTML = str+'نۆۋەتتە ھاسىللىنىۋاتقىنى '+current+'-ھۆججەت، تاماملىنىشى '+cent+'%';
				} else {
					child.style.color = 'blue';
					child.innerHTML = str+obj.dom.getAttribute('mktitle')+'ھاسىللاش تامام';
				}
				if(id) {
					obj.make(id, child);
