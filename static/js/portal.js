...
function block_delete_item(bid, itemid, itemtype, itemfrom, from) {
	var msg = itemtype==1 ? 'بۇ سانلىق مەلۇماتىنى ئۆچۈرەمسىز؟' : 'بۇ سانلىق مەلۇماتنى توسامسىز؟';
	if(confirm(msg)) {
...
function recommenditem_byblock(bid, id, idtype) {
	...
	if(editarea) {
		...
		if(!$('recommendback')) {
			...
			back.innerHTML = '<em id="recommendback" onclick="recommenditem_back()" class="cur1">&nbsp;&nbsp;&laquo;قايتىش</em>';
...
function toggleSettingShow() {
	if(!$('tbody_setting').style.display) {
		...
		$('a_setting_show').innerHTML = 'تەڭشەڭ تۈرىنى يېيىش';
	} else {
		$('tbody_setting').style.display = '';
		$('a_setting_show').innerHTML = 'تەڭشەڭ تۈرىنى يېغىش';
	}
	doane();
}
...
function checkblockname(form) {
	if(!(trim(form.name.value) > '')) {
		showDialog('قېلىپ كۆرسەتمىسىنى يېزىڭ', 'error', null, function(){form.name.focus();});
		return false;
	}
	if(form.summary && form.summary.value) {
		...
		if(tag) {
			showBlockSummary();
			form.summary.focus();
			showDialog('ئىختىيارى مەزمۇن خاتا，HTMLكودى：'+tag+' خەتكۈچ ماسلاشمىدى', 'error', null, function(){form.summary.select();});
			return false;
		}
	}
		...
		for(var i = 0; i < tags.length; i++) {
			...
			if((!preTag && endTag) || (preTag && !endTag) || preTag.length != endTag.length) {
				if(returnValue) {
					...
					showDialog('HTMLكودى：'+tag+' خەتكۈچ ماسلاشمىدى', 'error', null, fn, true, fn);
					return false;
				}
			}
		}
...
function blockconver(ele,bid) {
	if(ele && bid) {
		if(confirm('قېلىپ تۈرىنى دىن '+ele.options[0].innerHTML+'غا ئايلاندۇرامسىز '+ele.options[ele.selectedIndex].innerHTML)) {
...
function strLenCalc(obj, checklen, maxlen) {
	...
	if(curlen <= maxlen) {
		checklen.innerHTML = ' <b>'+(curlen)+'</b> ھەرپ كىرگۈزۈلدى';
		return true;
	} else {
		checklen.innerHTML = ' <b style="color:red">'+(curlen - maxlen)+'</b> ھەرپ ئېشىپ كەتتى';
		return false;
	}
}
...
function check_itemdata_lentgh(form) {
	if(form.title && (!strLenCalc(form.title, "titlechk", form.title.getAttribute('_maxlength')) || !form.title.value)) {
		form.title.focus();
		showDialog('ماۋزۇ ئۇزۇنلۇقى خاتا', 'error', null, function(){form.title.select();});
		return false;
	}
	if(form.summary && !strLenCalc(form.summary, "summarychk", form.summary.getAttribute('_maxlength'))) {
		form.summary.focus();
		showDialog('چۈشەندۈرۈش ئۇزۇنلۇقى خاتا', 'error', null, function(){form.summary.select();});
		return false;
	}
	return true;
}