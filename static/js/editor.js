...
function initEditor() {
	...
	for(var i = 0; i < buttons.length; i++) {
		if(buttons[i].id.indexOf(editorid + '_') != -1) {
			...
			if(buttons[i].id.substr(buttons[i].id.indexOf('_') + 1) == 'fullswitcher') {
				buttons[i].innerHTML = !editorisfull ? 'پۈتۈن ئېكران' : 'قايتىش';
				buttons[i].onmouseover = function(e) {setEditorTip(editorisfull ? 'تەھرىرلىگۈچ چوڭلۇقىنى ئەسلىگە قايتۇرۇش' : 'پۈتۈن ئېكران ھالىتىدە تەھرىرلەش');};
				buttons[i].onclick = function(e) {editorfull();doane();}
			} else if(buttons[i].id.substr(buttons[i].id.indexOf('_') + 1) == 'simple') {
				buttons[i].innerHTML = !simplodemode ? 'دائىملىق' : 'ئالاھىدە';
	...
	if($(editorid + '_fullswitcher') && BROWSER.ie && BROWSER.ie < 7) {
		$(editorid + '_fullswitcher').onclick = function () {
			showDialog('تور كۆرگۈچىڭىز بۇ ئىقتىدارنى قوللىمايدۇ،نەشرىنى كۆتۈرۈڭ', 'notice', 'دوستانە ئەسكەرتىش');
		};
...
function savedataTime() {
	if(!autosave) {
		$(editorid + '_svdsecond').innerHTML = '<a title="باسسىڭىز ئاپتوماتىك ساقلاش قوزغىلىدۇ" href="javascript:;" onclick="setAutosave()">ئاپتوماتىك ساقلاش قوزغىتىش</a> ';
		return;
	}
	if(!savedatac) {
		savedatac = savedataInterval;
		...
		h = h < 10 ? '0' + h : h;
		m = m < 10 ? '0' + m : m;
		setEditorTip('سانلىق مەلۇمات ' + h + ':' + m + ' دە ساقلاندى');
	}
	$(editorid + '_svdsecond').innerHTML = '<a title="چەكسىڭىز ئاپتوماتىك ساقلاش تاقىلىدۇ" href="javascript:;" onclick="setAutosave()">' + savedatac + ' سېكونتتىن كېيىن ساقلىنىدۇ</a> ';
	savedatac -= 10;
}

function setAutosave() {
	autosave = !autosave;
	setEditorTip(autosave ? 'ئاپتوماتىك ساقلاش ئىقتىدارى قوزغىتىلدى' : 'ئاپتوماتىك ساقلاش ئىقتىدارى تاقالدى');
...
function editorfull(op) {
	...
	$(editorid + '_fullswitcher').innerHTML = editorisfull ? 'قايتىش' : 'پۈتۈن ئېكران';
	initesbar();
}
...
function editorsimple() {
	if($(editorid + '_body').className == 'edt') {
		v = 'none';
		$(editorid + '_simple').innerHTML = 'ئالاھىدە';
		...
		v = '';
		$(editorid + '_simple').innerHTML = 'دائىملىق';
...
function checklength(theform) {
	...
	showDialog('ھازىرقى ئۇزۇنلۇقى: ' + mb_strlen(message) + ' ھەرپ，' + (postmaxchars != 0 ? 'سىستېما چەكلىمىسى:' + postminchars + ' دىن ' + postmaxchars + ' ھەرپكىچە.' : ''), 'notice', 'ھەرپ ئېنىقلىمىسى');
}
...
function setEditorStyle() {
	if(wysiwyg) {
		...
		var nui_i_id = editorid + '_iframe';
		nuiKeyFunc(nui_i_id, null);/* Nurqut NUIedit for iframe by FinalDream 20121007*/
	} else {
...
function discuzcode(cmd, arg) {
	...
	if(in_array(cmd, ['sml', 'inserthorizontalrule', 'url', 'quote', 'code', 'free', 'hide', 'aud', 'vid', 'fls', 'beginning', 'attach', 'image', 'pasteword', 'index', 'postbg', 'password']) || typeof EXTRAFUNC['showEditorMenu'][cmd] != 'undefined' || cmd == 'tbl' || in_array(cmd, ['fontname', 'fontsize', 'forecolor', 'backcolor']) && !arg) {
		...
		if(txt = getSel()) {
			...
			while(listvalue = prompt('بىر جەدۋەل تۈرىنى تولدۇرۇڭ.\r\nبوش قالسا ياكى بىكار قىلىشنى چېكىپ جەدۋەلنى ئاياغلاشتۇرۇڭ', '')) {
				if(BROWSER.opera > 8) {
					listvalue = '\n' + '[*]' + listvalue;
		...
		loadData();
		setEditorTip('سانلىق مەلۇمات ئەسلىگە كەلتۈرۈلدى');
	} else if(cmd == 'svd') {
		saveData();
		setEditorTip('سانلىق مەلۇمات ساقلاندى');
	} else if(cmd == 'chck') {
		checklength(editorform);
	} else if(cmd == 'tpr') {
		if(confirm('راستىنلا بارلىق مەزمۇننى تازىلامسىز؟')) {
			clearContent();
		}
	} else if(cmd == 'downremoteimg') {
		showDialog('<div id="remotedowninfo"><p class="mbn">يىراق مۇساپىلىك ھۆججەت چۈشۈرىلىۋاتىدۇ،تەخىر قىلىڭ...</p><p><img src="' + STATICURL + 'image/common/uploading.gif" alt="" /></p></div>', 'notice', '', null, 1);
...
function setContext(cmd) {
	...
	fs = fs && cmd != 'clear' ? fs : 'خەت نۇسخىسى';
...
function showEditorMenu(tag, params) {
	...
	if(menu) {
		...
		switch(tag) {
			case 'url':
				str = 'ئۇلىنىش ئادىرسىنى كىرگۈزۈڭ<br /><input type="text" id="' + ctrlid + '_param_1" style="width: 98%" value="" class="px" />'+
					(selection ? '' : '<br />خەتلىك ئۇلىنىش ئادىرسىنى كىرگۈزۈڭ<br /><input type="text" id="' + ctrlid + '_param_2" style="width: 98%" value="" class="px" />');
				break;
			case 'forecolor':
			...
			case 'postbg':
				...
				break;
			case 'password':
				str = '<p class="pbn">يازما پارولىنى كىرگۈزۈڭ: <input type="text" id="' + ctrlid + '_param_1" size="10" value="" class="px" /></p>';
				break;
			case 'code':
				if(wysiwyg) {
			...
			case 'free':
				...
				var lang = {'quote' : 'قىستۇرىدىغان نەقىلنى كىرگۈزۈڭ', 'code' : 'قىستۇرىدىغان كودنى كىرگۈزۈڭ', 'hide' : 'يوشۇرىدىغان ئۇچۇر مەزمۇننى كىرگۈزۈڭ', 'free' : 'يازما سېتىلىش قىممىتىنى بەلگىلىگەن بولسىڭىز،سېتىۋېلىشتىن بۇرۇنقى كۆرۈنىدىغان مەزمۇننى كىرگۈزۈڭ'};
				str += lang[tag] + ':<br /><textarea id="' + ctrlid + '_param_1" style="width: 98%" cols="50" rows="5" class="txtarea"></textarea>' +
					(tag == 'hide' ? '<br /><label><input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_1" class="pc" checked="checked" />زىيارەت قىلغۇچى ئىنكاس قايتۇرغاندىن كېيىن كۆرۈنىدۇ</label><br /><label><input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_2" class="pc" />زىيارەتچى جۇغلانمىسى </label> <input type="text" size="3" id="' + ctrlid + '_param_2" class="px pxs" /> بولغاندا كۆرەلەيدۇ<br /><br /><label>كۈچكە ئىگە كۈن سانى:</label> <input type="text" size="3" id="' + ctrlid + '_param_3" class="px pxs" /> <br />يازما يوللانغان ۋاقت بىلەن بولغان ئارلىقى بۇ كۈن سانىدىن كۆپ بولغاندا خەتكۈچ ئۈنۈمىدىن قالىدۇ' : '');
				break;
			case 'tbl':
				str = '<p class="pbn">جەدۋەل قۇرسانى:<input type="text" id="' + ctrlid + '_param_1" size="2" value="2" class="px" /> &nbsp; جەدۋەل ئىستون سانى <input type="text" id="' + ctrlid + '_param_2" size="2" value="2" class="px" /></p><p class="pbn">جەدۋەل كەڭلىكى <input type="text" id="' + ctrlid + '_param_3" size="2" value="" class="px" /> &nbsp; تەگلىك رېڭى: <input type="text" id="' + ctrlid + '_param_4" size="2" class="px" onclick="showColorBox(this.id, 2)" /></p><p class="xg2 pbn" style="cursor:pointer" onclick="showDialog($(\'tbltips_msg\').innerHTML, \'notice\', \'كىچىك ئەسكەرتمە\', null, 0)"><img id="tbltips" title="كىچىك ئەسكەرتمە" class="vm" src="' + IMGDIR + '/info_small.gif"> جەدۋەل ئەسكەرتىشى يېزىش</p>';
				str += '<div id="tbltips_msg" style="display: none">“[tr=رەڭ]” قۇر تەگلىكى بەلگىلەش<br />“[td=كەڭلىك]” ئىستون كەڭلىكى بەلگىلەش<br />“[td=كەڭلىك،قۇرئاتلاش ئارلىقى،ئىستون ئارلىقى]” بەلگىلەنگەن قۇر كەڭلىكى<br /><br />جەدۋەل مىسال ئۈلگىسى<div class=\'xs0\' style=\'margin:0 5px\'>[table]<br />Name:|Discuz!<br />Version:|X1<br />[/table]</div>“|”بىلەن قۇر ئايرىڭ، جەدۋەلدە “|” بار بولسا “\\|” نى ئالماشتۇرۇپ ئىشلىتىشكە بولىدۇ، قۇر ئالماشتۇرۇشتا  “\\n” نى ئالماشتۇرۇشقا بولىدۇ</div>';
				break;
			case 'aud':
				str = '<p class="pbn">مۇزىكا ھۆججىتى ئادىرسىنى كىرگۈزۈڭ</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p><p class="xg2 pbn">wma mp3 ra rm قاتارلىق مۇزىكا ھۆججەتلىرىنى قوللايدۇ<br />مەسىلەن: http://server/audio.wma</p>';
				break;
			case 'vid':
				str = '<p class="pbn">سىن ئادېرسىنى كىرگۈزۈڭ:</p><p class="pbn"><input type="text" value="" id="' + ctrlid + '_param_1" style="width: 220px;" class="px" /></p><p class="pbn">كەڭلىكى: <input id="' + ctrlid + '_param_2" size="5" value="500" class="px" /> &nbsp; ئېگىزلىكى: <input id="' + ctrlid + '_param_3" size="5" value="375" class="px" /></p><p class="xg2 pbn">优酷、土豆、56、酷6 قاتارلىقلارنىڭ ئادرېسىنى قوللايدۇ<br />wmv avi rmvb mov swf flv فورماتلىرىنى قوللايدۇ<br />مەسىلەن: http://server/movie.wmv</p>';
				break;
			case 'fls':
				str = '<p class="pbn">Flash ئادېرسىنى كىرگۈزۈڭ:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p><p class="pbn">كەڭلىكى: <input id="' + ctrlid + '_param_2" size="5" value="" class="px" /> &nbsp; ئېگىزلىكى: <input id="' + ctrlid + '_param_3" size="5" value="" class="px" /></p><p class="xg2 pbn">swf flv قاتارلىق Flash ئادرېسلارنى قوللايدۇ<br />مەسىلەن: http://server/flash.swf</p>';
				break;
			case 'beginning':
				str = '<p class="pbn">باشلىنىدىغان ھەرىكەتلىك تەسۋىر Flash ياكى رەسىمنىڭ ئادرېسىنى كىرگۈزۈڭ :</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p>';
				str += '<p class="pbn">ئۇلىنىش ئادرېسى :</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_2" class="px" value="" style="width: 220px;" /></p>';
				str += '<p class="pbn"> كەڭلىكى :<input id="' + ctrlid + '_param_3" size="5" value="" class="px" /> &nbsp; ئېگىزلىكى : <input id="' + ctrlid + '_param_4" size="5" value="" class="px" /></p>';
				str += '<p class="pbn">توختاش سېكۇنتى : <input id="' + ctrlid + '_param_8" size="5" value="" class="px" /></p>';
				str += '<p class="pbn">ئۈنۈمى :  </p><p class="pbn"><input id="' + ctrlid + '_param_7" type="radio" name="effect" checked />يوق &nbsp; <input id="' + ctrlid + '_param_5" type="radio" name="effect" />سۇس ئالمىشىش &nbsp; <input id="' + ctrlid + '_param_6" type="radio" name="effect" />ئىچىش-يىپىش</p>';
				str += '<p class="xg2 pbn"> swf flv jpg gif png قاتارلىق ئادرېسلارنى قوللايدۇ <br />كەڭلىك ئېگىزلىك دائىرسى : كەڭلىكگى  400~1024  ئېگىزلىگى 300~600  <br />مىسال :  http://server/flash.swf</p>';
				break;
			case 'pasteword':
				stitle = 'Wordتىن چاپلاش';
				str = '<p class="px" style="height:300px"><iframe id="' + ctrlid + '_param_1" frameborder="0" style="width:100%;height:100%" onload="this.contentWindow.document.body.style.width=\'550px\';this.contentWindow.document.body.contentEditable=true;this.contentWindow.document.body.focus();this.onload=null"></iframe></p><p class="xg2 pbn">Ctrl+Vنى بېسىپ Word تىكى مەزمۇننى ئۈستى قىسىمغا چاپلاڭ</p>';
				menuwidth = 600;
				menupos = '00';
				menutype = 'win';
				break;
			case 'index':
				stitle = 'تېما مۇندەرىجىسى قۇرۇش ';
				str = '<p class="pbn">[index]<br />\n\
					[#<span class="xi1">بەت نومۇرى</span>]<span class="xi1">ماۋزۇ</span> &nbsp;&nbsp;<span class="xg1">بەلگىلەنگەن بەتكە ئاتلاش</span><br />\n\
					<span class="xi1">*</span>[#<span class="xi1">tid,pid</span>]<span class="xi1">ماۋزۇ</span> &nbsp;&nbsp;<span class="xg1">بەلگىلەنگەن تېمىغا ئاتلاش</span><br />\n\
					[/index]<br />\n\
					<br />\n\
					<span class="xi1">بەت نومۇرى</span> &nbsp;&nbsp;<span class="xg1"> [page] نۆۋەتتىكى يازمىڭىزدا مەسىلە كۆرۈلدى</span><br />\n\
					<span class="xi1">tid,pid</span> &nbsp;&nbsp;<span class="xg1">يازما TID بىلەن PID</span><br />\n\
					<span class="xi1">*</span> &nbsp;&nbsp;<span class="xg1">تۇنجى قۇرنى ئىچىگە تارتىش</span></p>';
				break;
			default:
				...
				if(!str) {
					...
					for(var i = 1; i <= params; i++) {
						if(i != params || !haveSel) {
							str += (promptlang[i - 1] ? promptlang[i - 1] : 'ىنچى ' + i + ' پارامېتىرنى كىرگۈزۈڭ:') + '<br /><input type="text" id="' + ctrlid + '_param_' + i + '" style="width: 98%" value="" class="px" />' + (i < params ? '<br />' : '');
						}
					}
				}
				break;
		...
		if(menupos == '00') {
			...
			s = '<table width="100%" cellpadding="0" cellspacing="0" class="fwin"><tr><td class="t_l"></td><td class="t_c"></td><td class="t_r"></td></tr><tr><td class="m_l">&nbsp;&nbsp;</td><td class="m_c">'
				+ '<h3 class="flb"><em>' + stitle + '</em><span><a onclick="hideMenu(\'\', \'win\');return false;" class="flbc" href="javascript:;">تاقاش</a></span></h3><div class="c">' + str + '</div>'
				+ '<p class="o pns"><button type="submit" id="' + ctrlid + '_submit" class="pn pnc"><strong>يوللاش</strong></button></p>'
				+ '</td><td class="m_r"></td></tr><tr><td class="b_l"></td><td class="b_c"></td><td class="b_r"></td></tr></table>';
		} else {
			s = '<div class="p_opt cl"><span class="y" style="margin:-10px -10px 0 0"><a onclick="hideMenu();return false;" class="flbc" href="javascript:;">تاقاش</a></span><div>' + str + '</div><div class="pns mtn"><button type="submit" id="' + ctrlid + '_submit" class="pn pnc"><strong>يوللاش</strong></button></div></div>';
		}
		menu.innerHTML = s;
