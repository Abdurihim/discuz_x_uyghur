...
function _checksec(type, idhash, showmsg, recall, modid) {
	...
	x.get('misc.php?mod=sec' + type + '&action=check&inajax=1&modid=' + modid + '&idhash=' + idhash + '&secverify=' + (BROWSER.ie && document.charset == 'utf-8' ? encodeURIComponent(secverify) : secverify), function(s){
		...
		if(s.substr(0, 7) == 'succeed') {
			...
			if(showmsg) {
				if(type == 'code') {
					showError('تەستىق نومۇر خاتا،قايتا كىرگۈزۈڭ');
				} else if(type == 'qaa') {
					showError('تەستىق سۇئالنىڭ جاۋابى خاتا،قايتا تولدۇرۇڭ');
				}
				recall(0);
			}
		}
...
function _copycode(obj) {
	...
	setCopy(BROWSER.ie ? obj.innerText.replace(/\r\n\r\n/g, '\r\n') : obj.textContent, 'كود كۆچۈرۈش تاختىسىغا كۆچۈرۈلدى');
}
...
function _setCopy(text, msg){
	if(BROWSER.ie) {
		...
		if(r) {
			...
			showDialog('<div class="c"><div style="width: 200px; text-align: center;">كۆچۈرۈش ئوڭۇشلۇق بولمىدى،زىيارەتكە يول قويۇشنى تاللاڭ”</div></div>', 'alert');
		}
	} else {
		var msg = '<div class="c"><div style="width: 200px; text-align: center; text-decoration:underline;">بۇ يەرنى بېسىپ كۆچۈرۈڭ</div>' +
...
function _showselect(obj, inpid, t, rettype) {
	...
	if(!obj.id) {
		...
		s = '';
		if(!t) {
			s += showselect_row(inpid, 'بىر كۈن', 1, 0, rettype);
			s += showselect_row(inpid, 'بىر ھەپتە', 7, 0, rettype);
			s += showselect_row(inpid, 'بىر ئاي', 30, 0, rettype);
			s += showselect_row(inpid, 'ئۈچ ئاي', 90, 0, rettype);
			s += showselect_row(inpid, 'ئىختىيارى', -2);
		} else {
			if($(t)) {
				...
				s += showselect_row(inpid, 'ئىختىيارى', -1);
			} else {
				s += '<a href="javascript:;" onclick="$(\'' + inpid + '\').value = \'0\'">مەڭگۈ</a>';
				s += showselect_row(inpid, 'بىر ھەپتە', 7, 1, rettype);
				s += showselect_row(inpid, 'ئىككى ھەپتە', 14, 1, rettype);
				s += showselect_row(inpid, 'بىر ئاي', 30, 1, rettype);
				s += showselect_row(inpid, 'ئۈچ ئاي', 90, 1, rettype);
				s += showselect_row(inpid, 'يېرىم يىل', 182, 1, rettype);
				s += showselect_row(inpid, 'بىر يىل', 365, 1, rettype);
				s += showselect_row(inpid, 'ئىختىيارى', -1);
			}
		}
		$(div.id).innerHTML = s;
...
function _zoom(obj, zimg, nocover, pn, showexif) {
	...
	if(!menu && !pn) {
		...
		menu.id = menuid;
		if(cover) {
			menu.innerHTML = '<div class="zoominner" id="' + menuid + '_zoomlayer" style="display:none"><p><span class="y"><a id="' + menuid + '_imglink" class="imglink" target="_blank" title="يېڭى كۆزنەكتە ئېچىش">يېڭى كۆزنەكتە ئېچىش</a><a id="' + menuid + '_adjust" href="javascipt:;" class="imgadjust" title="ئەمىلىي چوڭلىقى">ئەمىلىي چوڭلىقى</a>' +
				'<a href="javascript:;" onclick="hideMenu()" class="imgclose" title="تاقاش">تاقاش</a></span>مائۇس غالتىكى ئارقىلىق رەسىمنى چوڭايتقىلى ۋە كىچىكلەتكىلى بولىدۇ</p>' +
				'<div class="zimg_p" id="' + menuid + '_picpage"></div><div class="hm" id="' + menuid + '_img"></div></div>';
		} else {
			menu.innerHTML = '<div class="popupmenu_popup" id="' + menuid + '_zoomlayer" style="width:auto"><span class="right y"><a href="javascript:;" onclick="hideMenu()" class="flbc" style="width:20px;margin:0 0 2px 0">تاقاش</a></span>مائۇس غالتىكى ئارقىلىق رەسىمنى چوڭايتقىلى ۋە كىچىكلەتكىلى بولىدۇ<div class="zimg_p" id="' + menuid + '_picpage"></div><div class="hm" id="' + menuid + '_img"></div></div>';
		}
	...
	if(typeof zoomgroup == 'object' && zoomgroup[obj.id] && typeof aimgcount == 'object' && aimgcount[zoomgroup[obj.id]]) {
		...
		if(authorlength > 1) {
			...
			if(authorcurrent !== '') {
				...
				picpage += ' <div id="zimg_prev" onmouseover="dragMenuDisabled=true;this.style.backgroundPosition=\'0 50px\'" onmouseout="dragMenuDisabled=false;this.style.backgroundPosition=\'0 -100px\';" onclick="_zoom_page(\'' + paid + '\', ' + (showexif ? 1 : 0) + ')" class="zimg_prev"><strong>ئالدىنقىسى</strong></div> ';
				paid = authorcurrent < authorlength - 1 ? authorimgs[authorcurrent + 1] : authorimgs[0];
				picpage += ' <div id="zimg_next" onmouseover="dragMenuDisabled=true;this.style.backgroundPosition=\'100% 50px\'" onmouseout="dragMenuDisabled=false;this.style.backgroundPosition=\'100% -100px\';" onclick="_zoom_page(\'' + paid + '\', ' + (showexif ? 1 : 0) + ')" class="zimg_next"><strong>كېيىنكىسى</strong></div> ';
			}
			if(picpage) {
...
function _showColorBox(ctrlid, layer, k, bgcolor) {
	...
	if(!$(ctrlid + '_menu')) {
		...
		var colortexts = ['قارا', 'بېغىررەڭ', 'قارامتۇل زەيتۇن يېشىل رەڭ', 'قارامتۇل يېشىل', 'قارامتۇل كۆك كۈل رەڭ', 'كۆكۈش رەڭ', 'قارا كۆك', 'ئىسلىق يېشىل', 'قارامتۇل قىزىل', 'قارامتۇل ئاپلىسىن سېرىقى', 'زەيتۇن رەڭ', 'يېشىل', 'توق يېشىل', 'كۆك رەڭ', 'كۈلرەڭ', 'قارامتۇل كۈلرەڭ', 'قىزىل', 'مېغىزرەڭ', 'سېرىق يېشىل', 'دېڭىز يېشىل', 'ئالا يېشىل', 'توق كۆك', 'سۆسۈن', 'كۈل رەڭ', 'قىزىل سۆسۈن', 'ئاپلىسىن رەڭ', 'سېرىق', 'توق ئېپلىسىن', 'يېشىل كۆك', 'قېنىق ئاسمان كۆى', 'قارامتۇل سۆسۈن', 'كۈنۈش رەڭ', 'ھال رەڭ', 'ئاچ سېرىق', 'لىمون', 'ياپيېشىل', 'ئېنىق يېشىل', 'يورۇق كۆك', 'نەشپۈت رەڭ', 'ئاق رەڭ'];
		var str = '';
		for(var i = 0; i < 40; i++) {
...
function _showCreditmenu() {
	if(!$('extcreditmenu_menu')) {
		...
		menu.innerHTML = '<div class="p_opt"><img src="'+ IMGDIR + '/loading.gif" width="16" height="16" class="vm" /> سەل ساقلاڭ...</div>';
		$('append_parent').appendChild(menu);
...
function _setShortcut() {
	$('shortcuttip').onclick = function() {
		var msg = '1. ئاۋۋال "' + '<a href="javascript:;" class="xi2 xw1" ';
		msg += 'onclick="this.href = \'forum.php?mod=misc&action=shortcut\';this.click();saveUserdata(\'setshortcut\', 1);"';
		msg += '>تېزلەتمىنى ئۈستەل يۈزىگە چۈشۈرۈڭ</a>' + '" ، چۈشۈرگەندىن كېيىن، سېستىما ئۈستەل يۈزىگە يۆتكەڭ. <br />';
		msg += '2. ئاندىن "' + '<a href="forum.php?mod=misc&action=shortcut&type=ico" class="xi2 xw1">';
		msg += 'ICO  سىنبەلگىسىنى چۈشۈرۈڭ </a>' + '"، چۈشۈرگەندىن كېيىن ، ئاۋۋالقى تېزلەتمىنىڭ  -> خاسلىق -> سىن بەلگىنى ئۆزگەرتىش، دېگەن جايدىن بۇ ICO سىنبەلگىنى تاللاڭ.';
		showDialog(msg, 'notice', 'ئۈستەليۈزى تېزلەتمە');
	};
