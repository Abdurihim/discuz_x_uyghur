...
drag.extend({
	'getBlocksTimer' : '',
	'blocks' : [],
	'blockDefaultClass' : [{'key':'شەكىل تاللاش','value':''},{'key':'رامكا ھەم يان ئارىلىق يوق، ','value':'cl_block_bm'},{'key':'شەكىل1','value':'xbs_1'},{'key':'شەكىل2','value':'xbs xbs_2'},{'key':'شەكىل3','value':'xbs xbs_3'},{'key':'شەكىل4','value':'xbs xbs_4'},{'key':'شەكىل5','value':'xbs xbs_5'},{'key':'شەكىل6','value':'xbs xbs_6'},{'key':'شەكىل7','value':'xbs xbs_7'}],
	'frameDefaultClass' : [{'key':'شەكىل تاللاش','value':''},{'key':'رامكا ھەم يان ئارىلىق يوق','value':'cl_frame_bm'},{'key':'رامكا يوق','value':'xfs xfs_nbd'},{'key':'شەكىل1','value':'xfs xfs_1'},{'key':'شەكىل2','value':'xfs xfs_2'},{'key':'شەكىل3','value':'xfs xfs_3'},{'key':'شەكىل4','value':'xfs xfs_4'},{'key':'شەكىل5','value':'xfs xfs_5'}],
	setDefalutMenu : function () {
		this.addMenu('default','ماۋزۇ','drag.openTitleEdit(event)');
		this.addMenu('default','شەكىل','drag.openStyleEdit(event)');
		this.addMenu('default', 'ئۆچۈرۈش', 'drag.removeBlock(event)');
		this.addMenu('block', 'خاسلىق', 'drag.openBlockEdit(event)');
		this.addMenu('block', 'سانلىق مەلۇمات', 'drag.openBlockEdit(event,"data")');
		this.addMenu('block', 'يېڭىلاش', 'drag.blockForceUpdate(event)');
		this.addMenu('frame', 'چىقىرىش', 'drag.frameExport(event)');
		this.addMenu('tab', 'چىقىرىش', 'drag.frameExport(event)');
	},
	...
	openStyleEdit : function (e) {
		...
		var repeatarr = [{'key':'يايىمەن','value':'repeat'},{'key':'يايمايمەن','value':'no-repeat'},{'key':'توغرىسىغا يېيىش','value':'repeat-x'},{'key':'بوي يۆلىنىشىدە يېيىش','value':'repeat-y'}];
		var stylearr = [{'key':'شەقىل يوق','value':'none'},{'key':'سىزىق','value':'solid'},{'key':'نۇقتا سىزىق','value':'dotted'},{'key':'ئۈزۈك سىزىق','value':'dashed'}];
		var table = '<table class="tfm">';
		table += '<tr><th>خەت شەكلى</th><td><input type="text" id="fontsize" class="px p_fre vm" value="'+fontsize+'" size="2" />px <input type="text" id="fontcolor" class="px p_fre vm" value="'+fontcolor+'" size="2" />';
		table += getColorPalette(id+'_fontPalette', 'fontcolor' ,fontcolor)+'</td></tr>';
		table += '<tr><th>ئۇلىنىش</th><td><input type="text" id="linkfontsize" class="px p_fre vm" value="'+linkfontsize+'" size="2" />px <input type="text" id="linkcolor" class="px p_fre vm" value="'+linkcolor+'" size="2" />';
		...
		table += '<tr><th>يان رامكا</th><td><ul id="borderul" class="'+ulclass+'">';
		table += '<li><label>ئۈستى</label><select class="ps vm" id="bdtwidth" ><option value="">چوڭ-كىچىكلىكى</option>'+this.getOption(widtharr,bdtwidth)+'</select>';
		table += ' <select class="ps vm" id="bdtstyle" ><option value="">شەكىل</option>'+this.getOption(stylearr,bdtstyle)+'</select>';
		table += ' رەڭ <input type="text" id="bdtcolor" class="px p_fre vm" value="'+bdtcolor+'" size="7" />';
		table += getColorPalette(id+'_bdtPalette', 'bdtcolor' ,bdtcolor)+'</li>';

		table += '<li class="bordera mtn"><label>右</label><select class="ps vm" id="bdrwidth" ><option value="">چوڭ-كىچىكلىكى</option>'+this.getOption(widtharr,bdrwidth)+'</select>';
		table += ' <select class="ps vm" id="bdrstyle" ><option value="">شەكىل</option>'+this.getOption(stylearr,bdrstyle)+'</select>';
		table += ' رەڭ <input type="text" id="bdrcolor" class="px p_fre vm" value="'+bdrcolor+'" size="7" />';
		table += getColorPalette(id+'_bdrPalette', 'bdrcolor' ,bdrcolor)+'</li>';

		table += '<li class="bordera mtn"><label>下</label><select class="ps vm" id="bdbwidth" ><option value="">چوڭ-كىچىكلىكى</option>'+this.getOption(widtharr,bdbwidth)+'</select>';
		table += ' <select class="ps vm" id="bdbstyle" ><option value="">شەكىل</option>'+this.getOption(stylearr,bdbstyle)+'</select>';
		table += ' رەڭ <input type="text" id="bdbcolor" class="px p_fre vm" value="'+bdbcolor+'" size="7" />';
		table += getColorPalette(id+'_bdbPalette', 'bdbcolor' ,bdbcolor)+'</li>';

		table += '<li class="bordera mtn"><label>左</label><select class="ps vm" id="bdlwidth" ><option value="">چوڭ-كىچىكلىكى</option>'+this.getOption(widtharr,bdlwidth)+'</select>';
		table += ' <select class="ps vm" id="bdlstyle" ><option value="">شەكىل</option>'+this.getOption(stylearr,bdlstyle)+'</select>';
		table += ' رەڭ <input type="text" id="bdlcolor" class="px p_fre vm" value="'+bdlcolor+'" size="7" />';
		table += getColorPalette(id+'_bdlPalette', 'bdlcolor' ,bdlcolor)+'</li>';
		table += '</ul><p class="ptm"><label><input id="borderop" type="checkbox" value="1" class="pc"'+opchecked+' onclick="$(\'borderul\').className = $(\'borderul\').className == \'borderul\' ? \'borderula\' : \'borderul\'">ئايرىم-ئايرىم تەڭشەش</label></p></td></tr>';
		...
		table += '<tr><th>سىرتقى يۈز ئارىلىقى</th><td><div id="margindiv" class="'+ulclass+'"><span><label>ئۈستى</label> <input type="text" id="margint" class="px p_fre vm" value="'+margint+'" size="1"/>px </span>';
		table += '<span class="bordera"><label>ئوڭ</label> <input type="text" id="marginr" class="px p_fre vm" value="'+marginr+'" size="1" />px </span>';
		table += '<span class="bordera"><label>ئاستى</label> <input type="text" id="marginb" class="px p_fre vm" value="'+marginb+'" size="1" />px </span>';
		table += '<span class="bordera"><label>سول</label> <input type="text" id="marginl" class="px p_fre vm" value="'+marginl+'" size="1" />px</span>';
		table += '</div><p class="ptm"><label><input id="marginop" type="checkbox" value="1" class="pc"'+opchecked+' onclick="$(\'margindiv\').className = $(\'margindiv\').className == \'borderul\' ? \'borderula\' : \'borderul\'">ئايرىم-ئايرىم تەڭشەش</label></p></td></tr>';
		...
		if (objType == 1) {
			...
			table += '<tr><th>ئىچكى يۈز ئارىلىقى</th><td><div id="cmargindiv" class="'+ulclass+'"><span><label>ئۈستى</label> <input class="px p_fre" id="cmargint" value="'+cmargint+'" size="1" />px </span>';
			table += '<span class="bordera"><label>ئوڭ</label> <input class="px p_fre" id="cmarginr" value="'+cmarginr+'" size="1" />px </span>';
			table += '<span class="bordera"><label>ئاستى</label> <input class="px p_fre" id="cmarginb" value="'+cmarginb+'" size="1" />px </span>';
			table += '<span class="bordera"><label>سول</label> <input class="px p_fre" id="cmarginl" value="'+cmarginl+'" size="1" />px </span>';
			table += '</div><p class="ptm"><label><input id="cmarginop" type="checkbox" value="1" class="pc"'+opchecked+' onclick="$(\'cmargindiv\').className = $(\'cmargindiv\').className == \'borderul\' ? \'borderula\' : \'borderul\'"> ئايرىم-ئايرىم تەڭشەش</label></p></td></tr>';
		}
		table += '<tr><th>تەگ رەڭگى</th><td><input type="text" id="bgcolor" class="px p_fre vm" value="'+bgcolor+'" size="4" />';
		table += getColorPalette(id+'_bgcPalette', 'bgcolor' ,bgcolor)+'</td></tr>';
		table += '<tr><th>تەگ رەسىم</th><td><input type="text" id="bgimage" class="px p_fre vm" value="'+bgimage+'" size="25" /> <select class="ps vm" id="bgrepeat" >'+this.getOption(repeatarr,bgrepeat)+'</select></td></tr>';
		var classarr = objType == 1 ? this.blockDefaultClass : this.frameDefaultClass;
		table += '<tr><th>كۆرسىلگەنclass</th><td><input type="text" id="diyClassName" class="px p_fre" value="'+diyClassName+'" size="8" /> <select class="ps vm" id="bgrepeat" onchange="$(\'diyClassName\').value=this.value;" >'+this.getOption(classarr, diyClassName)+'</select></td></tr>';
		table += '</table>';

		var wname = objType ? 'قېلىپ' : 'رامكا';
		html = '<div class="c diywin" style="width:450px;position:relative;">'+table+'</div>';
		var h = '<h3 class="flb"><em>تەھرىرلەش'+wname+'شەكىل</em><span><a href="javascript:;" class="flbc" onclick="drag.closeStyleEdit(\''+id+'\');return false;" title="تاقاش">\n\
			تاقاش</a></span></h3>';
		var f = '<p class="o pns"><button onclick="drag.saveStyle(\''+id+'\');drag.closeStyleEdit(\''+id+'\');" class="pn pnc" value="true">\n\
			<strong>جەزملەشتۈرۈش</strong></button><button onclick="drag.closeStyleEdit(\''+id+'\')" class="pn" value="true"><strong>بىكار قىلىش</strong></button></p>';
		this.removeMenu(e);
	...
	openTitleEdit : function (e) {
		...
		var titlename = obj instanceof Block ? 'قېلىپ' : 'رامكا';
		var repeatarr = [{'key':'يايىمەن','value':'repeat'},{'key':'يايمايمەن','value':'no-repeat'},{'key':'توغرىسىغا يېيىش','value':'repeat-x'},{'key':'بوي يۆلىنىشىدە يېيىش','value':'repeat-y'}];
		...
		common += '<tr><th>تەگ رەسىم:</th><td><input type="text" id="titleBgImage" class="px p_fre" value="'+bgimage+'" /> <select class="ps vm" id="titleBgRepeat" >'+this.getOption(repeatarr,bgrepeat)+'</select></td></tr>';
		common += '<tr><th>تەگ رەڭ:</th><td><input type="text" id="titleBgColor" class="px p_fre" value="'+bgcolor+'" size="7" />';
		common += getColorPalette(fid+'bgPalette_0', 'titleBgColor' ,bgcolor)+'</td></tr>';
		if (obj instanceof Tab) {
			var switchArr = [{'key':'چېكىش','value':'click'},{'key':'سىيرىلىپ ئۆتۈش','value':'mouseover'}];
			var switchType = obj.titles['switchType'] ? obj.titles['switchType'][0] : 'click';
			common += '<tr><th>تۈر ئالماشتۇرۇش:</th><td><select class="ps" id="switchType" >'+this.getOption(switchArr,switchType)+'</select></td></tr>';
		}
		common += '</table><hr class="l">';
		var li = '';
		li += '<div id="titleInput_0"><table class="tfm"><tr><th>'+titlename+'ماۋزۇ:</th><td><input type="text" id="titleText_0" class="px p_fre" value="`title`" /></td></tr>';
		li += '<tr><th>ئۇلىنىش:</th><td><input type="text" id="titleLink_0" class="px p_fre" value="`link`" /></td></tr>';
		li += '<tr><th>رەسىم:</th><td><input type="text" id="titleSrc_0" class="px p_fre" value="`src`" /></td></tr>';
		li += '<tr><th>ئورۇن:</th><td><select id="titleFloat_0" class="ps vm"><option value="left" `left`>سولغا</option><option value="right" `right`>ئوڭغا</option></select>';
		li += '&nbsp;&nbsp;سىلجىش مىقدارى: <input type="text" id="titleMargin_0" class="px p_fre vm" value="`margin`" size="2" />px</td></tr>';
		li += '<tr><th>خەت شەكلى:</th><td><select class="ps vm" id="titleSize_0" ><option value="">چوڭ-كىچىكلىكى</option>`size`</select>';
		li += '&nbsp;&nbsp;رەڭ: <input type="text" id="titleColor_0" class="px p_fre vm" value="`color`" size="4" />';
		...
		var c = len + 1;
			html = '<div class="c diywin" style="width:450px;height:400px; overflow:auto;"><table cellspacing="0" cellpadding="0" class="tfm pns"><tr><th></th><td><button type="button" id="addTitleInput" class="pn" onclick="drag.addTitleInput('+c+');"><em>يېڭى ماۋزۇ قوشۇش</em></button></td></tr></table><div id="titleEdit">'+html+common+'</div></div>';
		var h = '<h3 class="flb"><em>تەھرىرلەش'+titlename+'ماۋزۇ</em><span><a href="javascript:;" class="flbc" onclick="drag.closeTitleEdit(\''+fid+'\');return false;" title="تاقاش">\n\
			تاقاش</a></span></h3>';
		var f = '<p class="o pns"><button onclick="drag.saveTitleEdit(\''+fid+'\');drag.closeTitleEdit(\''+fid+'\');" class="pn pnc" value="true">\n\
			<strong>جەزملەشتۈرۈش</strong></button><button onclick="drag.closeTitleEdit(\''+fid+'\')" class="pn" value="true"><strong>بىكار قىلىش</strong></button></p>';
		this.removeMenu(e);
	...
	removeBlock : function (e, flag) {
		...
		var obj = this.getObjByName(id);
		if (!flag) {
			if (!confirm('راستلا ئۆچۈرەمسىز؟ ئۆچۈرگەندىن كېيىن ئەسلىگە كەلتۈرگىلى بولمايدۇ')) return false;
		}
		if (obj instanceof Block) {
			this.delBlock(id);
		} else if (obj instanceof Frame) {
	...
	blockForceUpdate : function (e,all) {
		...
		bcontent.innerHTML = '<center>مەزمۇن قوشۇلىۋاتىدۇ...</center>';
		var x = new Ajax();
		x.get('portal.php?mod=portalcp&ac=block&op=getblock&forceupdate=1&inajax=1&bid='+bid+'&tpl='+document.diyform.template.value, function(s) {
			if(s.indexOf('errorhandle_') != -1) {
				...
				runslideshow();
				showDialog('كەچۈرۈڭ، سىزنىڭ قېلىپ قوشۇش ياكى تەھرىرلەش ھوقۇقىڭىز يوق', 'alert');
				doane();
			} else {
	...
	frameExport : function (e) {
		var flag = true;
		if (drag.isChange) {
			flag = confirm('ئۆزگەرتىش قىلدىڭىز، ساقلىغاندىن كېيىن چىقىرىڭ، بولمىسا بۇ قېتىم ساقلىغان سانلىق مەلۇمات كەم بولۇپ قالىدۇ ');
		}
		if (flag) {
			if ( typeof e == 'object') {
				e = Util.event(e);
	...
	endBlockForceUpdateBatch : function () {
		if($('allupdate')) {
			$('allupdate').innerHTML = 'مەشخۇلات تاماملاندى .';
	...
	getBlocks : function () {
		...
		if (this.blocks.length > 0) {
			...
			if($('allupdate')) {
				$('allupdate').innerHTML = 'جەمئىي<span style="color:blue">'+this.blocksLen+'</span>دانە قېلىپ,ھازىر<span style="color:red">'+cur+'</span>ىنچىسىنى يېڭىلاۋاتىدۇ,<span style="color:red">'+(parseInt(cur / this.blocksLen * 100)) + '%</span>سى تاماملاندى';
	...
	blockForceUpdateBatch : function (blocks) {
		...
		showDialog('<div id="allupdate" style="width:350px;line-height:28px;">يېڭىلاۋاتىدۇ...</div>','confirm','قېلىپ سانلىق مەلۇماتىنى يېڭىلاۋاتىدۇ', '', true, 'drag.endBlockForceUpdateBatch()');
		var wait = function() {
	...
	clearAll : function () {
		if (confirm('راستلا قۇرۇقدامسىز؟ قۇرۇقدىغاندىن كېيىن مەزمۇننى ئەسلىگە كەلتۈرگىلى بولمايدۇ')) {
	...
	createObj : function (e,objType,contentType) {
		if (objType == 'block' && !this.checkHasFrame()) {alert("ئەسكەرتىش: رامكا تېپىلمىدى، ئاۋۋال رامكا قوشۇڭ");spaceDiy.getdiy('frame');return false;}
		e = Util.event(e);
	...
	getFrameHtml : function (type) {
		...
		str += '<div id="'+id+'_title" class="'+this.titleClass+' '+this.frameTitleClass+'"><span class="'+this.titleTextClass+'">'+type+'رامكا</span></div>';
		var cols = type.split('-');
		var clsl='',clsc='',clsr='';
		clsl = ' frame-'+type+'-l';
		clsc = ' frame-'+type+'-c';
	...
	getTabHtml : function () {
		...
		var str = '<div id="'+id+'" class="'+className+'">';
		str += '<div id="'+id+'_title" class="'+titleClassName+'"><span class="'+this.titleTextClass+'">tab بەلگىسى</span></div>';
		...
		if(this.sampleMode) {
			...
			if (!this.isChange) {
				window.onbeforeunload = function() {
					return 'سىزنىڭ سانلىق-مەلۇماتىڭىزدا ئۆزگىرىش بولدى، چىكىنسىڭىز ساقلانمايدۇ';
				};
			}
			this.isChange = true;
	...
	goonDIY : function () {
		if ($('prefile').value == '1') {
			showDialog('<div style="line-height:28px;">داۋاملاشتۇرۇشنى باسسىڭىزۋاقىتلىق ساقلانغان سانلىق-مەلۇمات ۋە DIYئېچىلىدۇ，<br />ئۆچۈرۈشنى باسسىڭىز ۋاقىتلىق ساقلىغان سانلىق-مەلۇماتلار ئۆچۈرۈلىدۇ。</div>','confirm','DIY ۋاقتىنچە ساقلامسىز؟', function(){location.replace(location.href+'&preview=yes');}, true, 'spaceDiy.cancelDIY()', '', 'داۋاملاشتۇرۇش', 'ئۆچۈرۈش');
...
spaceDiy.extend({
		...
		if (!optype) {
			if (['portal/portal_topic_content', 'portal/list', 'portal/view'].indexOf(tplpre[0]) == -1) {
				if (document.diyform.template.value.indexOf(':') > -1 && !document.selectsave) {
					...
					showDialog('<form name="selectsave" action="" method="get"><label><input type="radio" value="0" name="savemod"'+schecked+' />مۇشۇ خىلدىكى بەتلەرنىڭ ھەممىسىگە ئىشلىتىش</label>\n\
					<label><input type="radio" value="1" name="savemod"'+dchecked+' />مۇشۇ بەتكىلا ئىشلىتىش</label></form>','notice', '', spaceDiy.save);
					return false;
				}
	...
	cancel : function () {
		...
		if (drag.isClearClose) {
			showDialog('<div style="line-height:28px;">داۋاملاشتۇرۇشنى باسسىڭىزۋاقىتلىق ساقلانغان سانلىق-مەلۇمات ۋە DIYئېچىلىدۇ，<br />ئۆچۈرۈشنى باسسىڭىز ۋاقىتلىق ساقلىغان سانلىق-مەلۇماتلار ئۆچۈرۈلىدۇ。</div>','confirm','DIY ۋاقتىنچە ساقلامسىز؟', function(){location.href = spaceDiy.cancelDiyUrl();}, true, function(){window.onunload=function(){spaceDiy.cancelDIY()};location.href = spaceDiy.cancelDiyUrl();});
		} else {
	...
	recover : function() {
		if (confirm('ئالدىنقى نەشىردە ساقلانغان نەتىجىگە قايتۇرامسىز؟')) {
			drag.clearClose();
