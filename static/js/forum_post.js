...
var STATUSMSG = {
	'-1' : 'ئىچكى خاتالىق',
	'0' : 'يوللاش مۇۋاپىقىيەتلىك بولدى',
	'1' : 'بۇ كىڭەيتىلگەن نامنى قوللىمايدۇ',
	'2' : 'مۇلازىمېتىر بۇنداق چوڭ قوشۇمچە ھۆججەت يوللاشنى قوللىمايدۇ',
	'3' : 'بۇ ئەزا گۇرپپىسىنىڭ بۇنداق چوڭ قوشۇمچە ھۆججەت يوللاش ھوقۇقى يوق',
	'4' : 'بۇ كىڭەيتىلگەن نامنى قوللىمايدۇ',
	'5' : 'ھۆججەت تۈرى چەكلىمىسى تۈپەيلى بۇنداق چوڭ ھۆججەت يوللىيالمايسىز',
	'6' : 'بۈگۈنكى قوشۇمچە يوللاش يوللاش چېكىڭىز توشتى',
	'7' : 'رەسىم ھۆججىتىنى تاللاڭ(' + imgexts + ')',
	'8' : 'قوشۇمچىنى ساقلىغىلى بولمىدى',
	'9' : 'مۇۋاپىق ھۆججەت يوللانمىدى',
	'10' : 'خاتا مەشغۇلات',
	'11' : 'بۈگۈنكى قوشۇمچە يوللاش يوللاش چېكىڭىز توشتى'
};
...
function checklength(theform) {
	...
	showDialog('ھازىرقى ئۇزۇنلۇقى: ' + mb_strlen(message) + ' ھەرپ，' + (postmaxchars != 0 ? 'سىستېما چەكلىمىسى: ' + postminchars + ' ھەرپتىن ' + postmaxchars + ' ھەرپكىچە。' : ''), 'notice', 'خەت سانىنى تەكشۈرۈش');
}
...
function validate(theform) {
	...
	if(($('postsubmit').name != 'replysubmit' && !($('postsubmit').name == 'editsubmit' && !isfirstpost) && theform.subject.value == "") || !sortid && !special && trim(message) == "") {
		showError('كەچۈرۈڭ، ماۋزۇ يەكى مەزمۇن كىرگۈزمىدىڭىز');
		return false;
	} else if(mb_strlen(theform.subject.value) > 180) {
		showError('ماۋزۇ 180 ھەرپتىن ئېشىپ كەتتى');
		return false;
	}
	...
	if(!disablepostctrl && !sortid && !special && ((postminchars != 0 && mb_strlen(message) < postminchars) || (postmaxchars != 0 && mb_strlen(message) > postmaxchars))) {
		showError('يازمىڭىزنىڭ خەت چېكى تەلەپكە لايىق ئەمەس。\n\nھازىرقى ئۇزۇنلۇقى: ' + mb_strlen(message) + ' ھەرپ\nسىستېما چېكى : ' + postminchars + ' ھەرپتىن ' + postmaxchars + ' ھەرپكىچە');
		return false;
	}
	if(UPLOADSTATUS == 0) {
		if(!confirm('يوللاشنى كۈتۈۋاتقان قوشۇمچە بار، يوللىمامسىز؟')) {
			return false;
		}
	} else if(UPLOADSTATUS == 1) {
		showDialog('يوللىنىۋاتقان قوشۇمچە بار، يوللىنىپ بولغاندىن كېيىن يازما ئاپتوماتىك يوللىنىدۇ...', 'notice');
		AUTOPOST = 1;
		return false;
	}
	if(isfirstpost && $('adddynamic') != null && $('adddynamic').checked && $('postsave') != null && isNaN(parseInt($('postsave').value)) && ($('readperm') != null && $('readperm').value || $('price') != null && $('price').value)) {
		if(confirm('ئوقۇش چەكلىمىسى تەڭشىدىڭىز ياكى تاۋار تېمىسى يوللىدىڭىز، ئاڭلىغۇچىلىرىڭىزغا يوللامسىز؟ ') == false) {
			return false;
		}
	}
		...
		if(seccodecheck || secqaacheck) {
			...
			if(secqaacheck) {
				...
				if(chkv.indexOf('loading') != -1) {
					...
					showError('دەلىللەش سۇئال-جاۋابىدا خاتالىق بار，قايتا تولدۇرۇڭ');
					chk = 0;
				}
			}
			if(seccodecheck) {
				...
				if(chkv.indexOf('loading') !== -1) {
					...
					showError('دەلىللەش نومۇرى خاتا، قايتا تولدۇرۇڭ');
					chk = 0;
				}
			}
			if(chk) {
...
function uploadNextAttach() {
	...
	var sizelimit = '';
	if(arr[4] == 'ban') {
		sizelimit = '(قوشۇمچە ھۆججەت تۈرى چەكلەنگەن)';
	} else if(arr[4] == 'perday') {
		sizelimit = '( ' + arr[5] + ' ھەرپتىن ئېشىپ كەتمەسلىكى كېرەك)';
	} else if(arr[4] > 0) {
		sizelimit = '( ' + arr[4] + ' ھەرپتىن ئېشىپ كەتمەسلىكى كېرەك)';
	}
...
function uploadAttach(curId, statusid, prefix, sizelimit) {
	...
	if(curId > 0) {
		...
		if(nextId == curId || in_array(statusid, [6, 8])) {
			...
			if(UPLOADFAILED > 0) {
				showDialog('قوشۇمچە يوللاش تامام،مۇۋاپىقىيەتلىك بولغىنى ' + UPLOADCOMPLETE + ' دانە，مەغلۇب بولغىنى ' + UPLOADFAILED + ' دانە:' + FAILEDATTACHS);
				FAILEDATTACHS = '';
			}
			UPLOADSTATUS = 2;
			for(var i = 0; i < AID[prefix ? 1 : 0] - 1; i++) {
			...
			if(AUTOPOST) {
				...
				showDialog('قوشۇمچە يوللاش تامام', 'right', null, null, 0, null, null, null, null, 3);
			}
	...
	$(prefix + 'cpdel_' + nextId).innerHTML = '<img src="' + IMGDIR + '/loading.gif" alt="يوللىنىۋاتىدۇ..." />';
	UPLOADSTATUS = 1;
...
function insertAttach(prefix, id) {
	...
	if(extensions != '' && (re.exec(extensions) == null || ext == '')) {
		...
		showError('كەچۈرۈڭ، كېڭەيتىلگەن نامى بۇنداق ھۆججەتنى يوللىغىلى بولمايدۇ');
		return;
	}
	if(prefix == 'img' && imgexts.indexOf(ext) == -1) {
		reAddAttach(prefix, id);
		showError('رەسىم ھۆججىتىنى يوللاڭ(' + imgexts + ')');
		return;
	}

	$(prefix + 'cpdel_' + id).innerHTML = '<a href="javascript:;" class="d" onclick="reAddAttach(\'' + prefix + '\', ' + id + ')">ئۆچۈرۈش</a>';
...
function updateAttach(aid) {
	...
	if(!objupdate.innerHTML) {
		...
		objupdate.innerHTML = '<input type="file" name="attachupdate[paid' + aid + ']"><a href="javascript:;" onclick="updateAttach(' + aid + ')">بىكار قىلىش</a>';
	} else {
		obj.style.display = '';
		objupdate.innerHTML = '';
	}
...
function updateattachnum(type) {
	...
	if(num) {
		if($(editorid + '_' + type)) {
			$(editorid + '_' + type).title = 'ئىچىدە ' + num + (type == 'image' ? ' دانە رەسىم' : ' دانە قوشۇمچە');
		}
		...
		if($(editorid + '_' + type)) {
			$(editorid + '_' + type).title = type == 'image' ? 'رەسىم' : 'قوشۇمچە';
		}
		if($(editorid + '_' + type + 'n')) {
...
function updateDownImageList(msg) {
	...
	if(msg == '') {
		showError('كەچۈرۈڭ، ۋاقتىنچە يىراق مۇساپىلىك قوشۇمچە يوق');
	} else {
		...
		switchImagebutton('imgattachlist');$('imgattach_notice').style.display = '';
		showDialog('يىراق مۇساپىلىك قوشۇمچىنى چۈشۈرۈش تامام!', 'right', null, null, 0, null, null, null, null, 3);
	}
}
...
function uploadWindowload() {
	...
	if(arr[0] == 'DISCUZUPLOAD' && arr[2] == 0) {
		...
		var sizelimit = '';
		if(arr[7] == 'ban') {
			sizelimit = '(قوشۇمچە تۈرى چەكلەنگەن)';
		} else if(arr[7] == 'perday') {
			sizelimit = '( ' + arr[8] + ' ھەرپتىن ئېشىپ كەتمىسۇن)';
		} else if(arr[7] > 0) {
			sizelimit = '( ' + arr[7] + ' ھەرپتىن ئېشىپ كەتمىسۇن)';
		}
...
function addpolloption() {
	if(curoptions < maxoptions) {
		...
		$('polloption_new').outerHTML = '<span>بېلەت تاشلاش چېكى توشتى'+maxoptions+'</span>';
	}
}
...
function attachoption(type, op) {
	if(!op) {
		...
		showDialog('<div id="unusedwin" class="c altw" style="overflow:auto;height:100px;">' + $('unusedlist_' + type).innerHTML + '</div>' +
			'<p class="o pns"><span class="z xg1"><label for="unusedwinchkall"><input id="unusedwinchkall" type="checkbox" onclick="attachoption(\'' + type + '\', 3)" checked="checked" />ھەممىنى تاللاش</label></span>' +
			'<button onclick="attachoption(\'' + type + '\', 1);hideMenu(\'fwin_dialog\', \'dialog\')" class="pn pnc"><strong>ئىشلىتىش</strong></button></p>', 'info', 'ئىشلىتىلمىگىگەن' + (type == 'attach' ? 'قوشۇمچە' : 'رەسىم'));
	} else if(op == 3) {
...
function getreplycredit() {
	...
	if(real_reply_credit > userextcredit) {
		$('replycredit').innerHTML = '<b class="xi1">ئىنكاس مۇكاپاتى يۇقىرى چېكى('+real_reply_credit+')</b>';
	} else {
		if(have_replycredit > 0 && real_reply_credit < 0) {
			$('replycredit').innerHTML = "<font class='xi1'>قايتۇرۇش"+Math.abs(real_reply_credit)+"</font>";
		} else {
...
function deleteThread() {
	if(confirm('يازمىنى ئۆچۈرەمسىز؟') != 0){
		$('delete').value = '1';
		$('postform').submit();
	}
}
