...
function checkPwdComplexity(firstObj, secondObj, modify) {
	...
	firstObj.onblur = function () {
		if(firstObj.value == '') {
			var pwmsg = !modifypwd ? 'پارول كىرگۈزۈڭ' : profileTips;
			if(pwlength > 0) {
				pwmsg += ', ئەڭ ئاز بولغاندا '+pwlength+' ھەرپ';
			}
	...
	firstObj.onkeyup = function () {
		if(pwlength == 0 || $(firstObj.id).value.length >= pwlength) {
			var passlevels = new Array('','ئاجىز','نورمال','كۈچلۈك');
			var passlevel = checkstrongpw(firstObj.id);
			errormessage(firstObj.id, '<span class="passlevel passlevel'+passlevel+'">پارول بىخەتەرلىك دەرىجىسى:'+passlevels[passlevel]+'</span>');
		}
	};
	secondObj.onblur = function () {
		if(secondObj.value == '') {
			errormessage(secondObj.id, !modifypwd ? 'پارولنى يەنە بىر قېتىم كىرگۈزۈڭ' : profileTips);
		}
		checkpassword(firstObj.id, secondObj.id);
	};
}
...
function addMailEvent(mailObj) {
	...
	mailObj.onblur = function () {
		if(mailObj.value == '') {
			errormessage(mailObj.id, 'ئېلخەت ئادېرسىڭىزنى تولدۇرۇڭ');
		}
...
function checkusername(id) {
	...
	if(username.match(/<|"/ig)) {
		errormessage(id, 'ئەزا ئىسمىدا سەزگۈر سۆزلەر بار');
		return;
	}
	var unlen = username.replace(/[^\x00-\xff]/g, "**").length;
	if(unlen < 3 || unlen > 15) {
		errormessage(id, unlen < 3 ? 'ئەزا ئىسمى 3 ھەرپتىن ئاز بولمىسۇن' : 'ئەزا ئىسمى 15 ھەرپتىن ئېشىپ كەتمىسۇن');
		return;
	}
	var x = new Ajax();
	$('tip_' + id).parentNode.className = $('tip_' + id).parentNode.className.replace(/ p_right/, '');
...
function checkpassword(id1, id2) {
	...
	if(pwlength > 0) {
		if($(id1).value.length < pwlength) {
			errormessage(id1, 'پارول بەك قىسقا، ئاز دېگەندە '+pwlength+' ھەرپ بولسۇن');
			return;
		}
	}
	if(strongpw) {
		...
		for(var i in strongpw) {
			...
			if(strongpw[i] === 2 && !$(id1).value.match(/[a-z]+/g)) {
				strongpw_error = true;
				strongpw_str[j] = 'كىچىك ھەرپ';
				j++;
			}
			if(strongpw[i] === 3 && !$(id1).value.match(/[A-Z]+/g)) {
				strongpw_error = true;
				strongpw_str[j] = 'چوڭ ھەرپ';
				j++;
			}
			if(strongpw[i] === 4 && !$(id1).value.match(/[^A-Za-z0-9]+/g)) {
				strongpw_error = true;
				strongpw_str[j] = 'ئالاھىدە بەلگىلەر';
				j++;
			}
		}
		if(strongpw_error) {
			errormessage(id1, 'پارول بەك ئاددىي '+strongpw_str.join('，'));
			return;
		}
	}
	errormessage(id2);
	if($(id1).value != $(id2).value) {
		errormessage(id2, 'ئىككى قېتىملىق پارول بىردەك ئەمەس');
	} else {
...
function checkemail(id) {
	...
	if(email.match(/<|"/ig)) {
		errormessage(id, 'ئېلخەت ئادېرسىدا سەزگۈر سۆزلەر بار');
		return;
	}
	var x = new Ajax();
	$('tip_' + id).parentNode.className = $('tip_' + id).parentNode.className.replace(/ p_right/, '');
...
function checkinvite() {
	...
	if(invitecode.match(/<|"/ig)) {
		errormessage('invitecode', 'تەكلىپ نومۇرىدا سەزگۈر سۆزلەر بار');
		return;
	}
	var x = new Ajax();
