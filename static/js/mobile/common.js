...
var page = {
	converthtml : function() {
		...
		var selector = '';
		if(lastpage) {
			selector += '<a id="select_a" style="margin:0 2px;padding:1px 0 0 0;border:0;display:inline-block;position:relative;width:100px;height:31px;line-height:27px;background:url('+STATICURL+'/image/mobile/images/pic_select.png) no-repeat;text-align:right;text-indent:20px;">';
			selector += '<select id="dumppage" style="position:absolute;left:0;top:0;height:27px;opacity:0;width:100px;">';
			for(var i=1; i<=lastpage; i++) {
				selector += '<option value="'+i+'" '+ (i == curpage ? 'selected' : '') +'>'+i+'-بەت</option>';
			}
			selector += '</select>';
			selector += '<span>'+curpage+'-بەت</span>';
		}

		$('div.pg').removeClass('pg').addClass('page').html('<a href="'+ prevpagehref +'">ئالدىنقى بەت</a>'+ selector +'<a href="'+ nextpagehref +'">كېيىنكى بەت</a>');
	...
	init : function() {
		...
		.on('touchmove', function(e) {
			...
			if(!status && $(window).scrollTop() <= 0) {
				...
				divobj = $(divobj);
				divobj.css({'position':'relative', 'margin-right':'-85px'});
				$('body').prepend(divobj);
				contentobj = document.createElement('div');
				contentobj = $(contentobj);
				contentobj.css({'position':'absolute', 'height':'30px', 'top': '-30px', 'left':'50%'});
				contentobj.html('<img src="'+ STATICURL + 'image/mobile/images/icon_arrow.gif" style="vertical-align:middle;margin-right:5px;-moz-transform:rotate(180deg);-webkit-transform:rotate(180deg);-o-transform:rotate(180deg);transform:rotate(180deg);"><span id="refreshtxt">ئاستىغا سىيرىسىڭىز يېڭىلىنىدۇ</span>');
			...
			if(status == true) {
				...
				if(pullheight >= 0 && pullheight < 150) {
					...
					if(reloadflag) {
						...
						contentobj.find('#refreshtxt').html('ئاستىغا سىيرىسىڭىز يېڭىلىنىدۇ');
					}
					reloadflag = false;
					...
					if(!reloadflag) {
						...
						contentobj.find('#refreshtxt').html('قويۇۋەتسىڭىز يېڭىلىنىدۇ');
					}
					reloadflag = true;
				}
			}
		...
		.on('touchend', function(e) {
			if(status == true) {
				if(reloadflag) {
					contentobj.html('<img src="'+ STATICURL + 'image/mobile/images/icon_load.gif" style="vertical-align:middle;margin-right:5px;">يۈكلىنىۋاتىدۇ...');
