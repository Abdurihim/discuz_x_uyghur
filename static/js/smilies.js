...
function smilies_switch(id, smcols, type, page, seditorkey) {
	...
	if(smilies_array[type].length > 2) {
		...
		smiliespage = '<div class="z"><a href="javascript:;" onclick="smilies_switch(\'' + id + '\', \'' + smcols + '\', ' + type + ', ' + prevpage + ', \'' + seditorkey + '\');doane(event);">ئالدىنقى بەت</a>' +
			'<a href="javascript:;" onclick="smilies_switch(\'' + id + '\', \'' + smcols + '\', ' + type + ', ' + nextpage + ', \'' + seditorkey + '\');doane(event);">كېيىنكى بەت</a></div>' +
...
function smilies_preview(seditorkey, id, obj, w) {
	...
	pos = spos['left'] >= mpos['left'] + $(id + '_data').offsetWidth / 2 ? '14' : '23';
