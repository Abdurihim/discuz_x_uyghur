	...
	Drag.prototype = {
		...
		checkEdit : function (ele) {
			...
			if (!$(id+'_edit')) {
				...
				dom.id = id+'_edit';
				dom.innerHTML = '<span id="'+id+'_edit_menu">تەھرىرلەش</span>';
				ele.appendChild(dom);
		...
		setClose : function () {
			if (!this.isChange) {
				window.onbeforeunload = function() {
					return 'ئۇچۇرلىرىڭىز ئۆزگەرتىلدى،چېكىنسىڭىز ئۇچۇرلار نورمال ساقلانمايدۇ';
				};
			}
			this.isChange = true;
		},
	...
	DIY.prototype = {
		...
		uploadSubmit : function (){
			if (document.uploadpic.attach.value.length<3) {
				alert('چىقارماقچى بولغان قوشۇمچە رەسىمنى تاللاڭ');
				return false;
			}
		...
		cancel : function () {
			var flag = false;
			if (this.isChange) {
				flag = confirm(this.cancelConfirm ? this.cancelConfirm : 'چېكىنسىڭىز بايىقى تەڭشەك ساقلانماسلىقى مۆمكىن،راستىنلا چېكىنەمسىز؟');
			}
