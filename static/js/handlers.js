...
function fileQueued(file) {
	try {
		...
		if(createQueue) {
			progress.setStatus("ساقلاۋاتىدۇ...");
		} else {
...
function fileQueueError(file, errorCode, message) {
	try {
		if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			...
			showDialog("سىز يوللىماقچى بولغان ھۆججەت سانى چەكتىن ئېشىپ كەتتى。\n"+(message === 0 ? "سىزنىڭ يوللاش چېكىڭىز توشتى。" : "سىز يەنە " + message + " ھۆججەت تاللىسىڭىز بولىدۇ"), 'notice', null, null, 0, null, null, null, null, sdCloseTime);
			return;
		}
		...
		switch (errorCode) {
			case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
				progress.setStatus("ھۆججەت چوڭ.");
				this.debug("Error Code: File too big, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
				break;
			case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
				progress.setStatus("قۇرۇق ھۆججەت يوللىغىلى بولمايدۇ");
				this.debug("Error Code: Zero byte file, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
				break;
			case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
				progress.setStatus("بۇ خىلدىكى ھۆججەت يوللاش چەكلەنگەن");
	...
	try {
		...
		if(this.customSettings.uploadSource == 'forum') {
			if(this.customSettings.uploadType == 'poll') {
				...
				if(aid > 0) {
					...
					if(typeof STATUSMSG[aid] == "string") {
						...
						progress.setStatus("بىكار قىلىش");
					}
					this.cancelUpload(file.id);
					progress.setCancelled();
					progress.toggleCancel(true, this);
			...
			if(parseInt(data.picid)) {
				...
				newTd = document.createElement("TD");
				newTd.className = 'd';
			newTd.innerHTML = 'رەسىم چۈشەندۈرۈلىشى<br/><textarea name="title['+data.picid+']" cols="40" rows="2" class="pt"></textarea>';
				newTr.appendChild(newTd);
				this.customSettings.imgBoxObj.appendChild(newTr);
			} else {
				showDialog('رەسىم يوللاش مەغلۇب بولدى', 'notice', null, null, 0, null, null, null, null, sdCloseTime);
			}
			$(file.id).style.display = 'none';
		} else if(this.customSettings.uploadType == 'blog') {
			var data = eval('('+serverData+')');
			...
			if(data.aid) {
				...
				showDialog('مەغلۇب بولدى', 'notice', null, null, 0, null, null, null, null, sdCloseTime);
