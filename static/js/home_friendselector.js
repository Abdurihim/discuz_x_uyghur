		...
		selectUserName:function(userName) {
			...
			if(userName != '') {
				...
				if(uid && typeof this.selectUser[uid] == 'undefined' || uid === 0 && typeof this.selectUser[userName] == 'undefined') {
					...
					this.selectNumber++;
					spanObj.innerHTML= '<a href="javascript:;" class="x" onclick="'+this.handleKey+'.delSelUser(\''+(spanObj.id)+'\');">ئۆچۈرۈش</a><em class="z" title="' + userName + '">' + userName + '</em><input type="hidden" name="users[]" value="'+userName+'" uid="uid'+uid+'" />';
					this.handleObj.parentNode.insertBefore(spanObj, this.handleObj);
					this.showObj.style.display = 'none';
				} else {
					alert('ئاللىبۇرۇن مەۋجۇت'+userName);
				}
			}
		},
			...
			if(uid){
				...
				if(typeof this.selectUser[uid] == 'undefined') {
					if(this.maxSelectNumber && this.selectNumber >= this.maxSelectNumber) {
			            alert('ئەڭ كۆپ بولغاندا'+this.maxSelectNumber+'ئەزا تاللىغىلى بولىدۇ');
			            return false;
			        }
					this.selectUser[uid] = this.dataSource[uid];
					this.selectNumber++;
