...
function fileQueued(file) {
	try {
		...
		if(createQueue) {
			progress.setStatus("يوللىنىۋاتىدۇ ، ساقلاڭ ...");
		} else {
...
function fileQueueError(file, errorCode, message) {
	try {
		if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			...
			showDialog("سىز يوللىغان ھۆججەت سانى چەكتىن ئىشىپ كەتتى .  \n"+(message === 0 ? "سىز يوللىغان ھۆججەت سانى چەكتىن ئىشىپ كەتتى . " : "سىز يەنە  " + message + " ھۆججەتنى تاللىيالايسىز "), 'notice', null, null, 0, null, null, null, null, sdCloseTime);
			return;
		}
		...
		switch (errorCode) {
			case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
				progress.setStatus("ھۆججەت بەك چوڭ .");
				this.debug("Error Code: File too big, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
				break;
			case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
				progress.setStatus("سىغىمى 0 خەتلىك بولغان ھۆججەتنى يوللاشقا بولمايدۇ .");
				this.debug("Error Code: Zero byte file, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
				break;
			case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
				progress.setStatus("بۇ شەكىلدىكى ھۆججەتنى يوللاش چەكلەنگەن .");
...
function uploadStart(file) {
	try {
		...
		var progress = new FileProgress(file, this.customSettings.progressTarget);
		progress.setStatus("يوللىنىۋاتىدۇ  ...");
		progress.toggleCancel(true, this);
		if(this.customSettings.uploadSource == 'forum') {
			var objId = this.customSettings.uploadType == 'attach' ? 'attachlist' : 'imgattachlist';
...
function uploadProgress(file, bytesLoaded, bytesTotal) {
	...
	try {
		...
		var progress = new FileProgress(file, this.customSettings.progressTarget);
		progress.setStatus("يوللىنىۋاتىدۇ ("+percent+"%)...");
	...
	try {
		...
		if(this.customSettings.uploadSource == 'forum') {
			if(this.customSettings.uploadType == 'poll') {
				...
				if(aid > 0) {
					...
					if(typeof STATUSMSG[aid] == "string") {
						...
						progress.setStatus("يوللاشنى بىكار قىلىش");
					}
					this.cancelUpload(file.id);
					progress.setCancelled();
					progress.toggleCancel(true, this);
			...
			if(parseInt(data.picid)) {
				...
				newTd = document.createElement("TD");
				newTd.className = 'd';
				newTd.innerHTML = 'رەسىم تەسۋىرلەش<br/><textarea name="title['+data.picid+']" cols="40" rows="2" class="pt"></textarea>';
				newTr.appendChild(newTd);
				this.customSettings.imgBoxObj.appendChild(newTr);
			} else {
				showDialog('رەسىم يوللاش مەغلۇپ بولدى', 'notice', null, null, 0, null, null, null, null, sdCloseTime);
			}
			$(file.id).style.display = 'none';
		} else if(this.customSettings.uploadType == 'blog') {
			var data = eval('('+serverData+')');
			...
			if(data.aid) {
				...
				showDialog('يوللاش مەغلۇپ بولدى', 'notice', null, null, 0, null, null, null, null, sdCloseTime);
			...
			progress.setStatus(" يوللاش تامام .");
			progress.toggleCancel(false);
		}
	} catch (ex) {
		this.debug(ex);
